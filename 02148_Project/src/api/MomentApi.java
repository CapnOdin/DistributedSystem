...
public class MomentApi {
	public static String[] loginMoment(String email, String pass) throws Exception {
		...
		String address = "", vikarGUID = "", str = "", ufprt = "";
		
		HttpClient httpClient = HttpClientBuilder.create().build();
		
		HttpGet getUFPRT = new HttpGet("https://www.moment.dk/staff/login");
		response = httpClient.execute(getUFPRT);
	
		str = IOUtils.toString(response.getEntity().getContent(), StandardCharsets.UTF_8);
		if(str.indexOf("name='ufprt'") > 5){
			ufprt = str.split("name='ufprt'")[1].split("value='")[1].split("'")[0];
 		}
		...
		try{
			...
			builder.addTextBody("ufprt", ufprt, ContentType.TEXT_PLAIN);
